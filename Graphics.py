...
class Graphics:
    def __init__(self, device_connected, window_size=(600, 400)):
        ...
        self.haptic = pygame.Rect(*self.screenHaptics.get_rect().center, 0, 0).inflate(self.haptic_width,
                                                                                       self.haptic_height)
        ...
        self.sim_k = 0.5  #0.5       ##Stiffness between cursor and haptic display
        self.sim_b = 0.8  #0.8       ##Viscous of the pseudohaptic display
    ...
    def sim_forces(self, pE, f, pM, mouse_k=None, mouse_b=None):
        ...
        if not self.device_connected:
            ...
            scale = self.window_scale / 1e3  #1e3
            ...
            cg = 255 - np.clip(np.linalg.norm(self.sim_k * 2 * diff / self.window_scale) * 255 * 20, 0, 255)
            cb = 255 - np.clip(np.linalg.norm(self.sim_k * 2 * diff / self.window_scale) * 255 * 20, 0, 255)
    ...
    def render(self, pA0, pB0, pA, pB, pE, f, pM):
        ...
        if self.show_debug:
            ...
            self.debug_text += "fe: " + str(np.round(f[0], 4)) + "," + str(np.round(f[1], 4)) + "] "
